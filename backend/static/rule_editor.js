...
function getForm(){
  return{
    label_name:document.getElementById('label-name').value.trim(),
    from_contains:document.getElementById('from-contains').value.trim(),
    subject_contains:document.getElementById('subject-contains').value.trim(),
    body_contains:document.getElementById('body-contains').value.trim(),
    is_active:document.getElementById('is-active').checked,
    mark_as_read:document.getElementById('mark-as-read').checked,
    keep_in_inbox:document.getElementById('keep-in-inbox').checked,
    star_email:document.getElementById('star-email').checked,
  };
}

function setForm(r){
  document.getElementById('label-name').value=r.label_name||'';
  document.getElementById('from-contains').value=r.from_contains||'';
  document.getElementById('subject-contains').value=r.subject_contains||'';
  document.getElementById('body-contains').value=r.body_contains||'';
  document.getElementById('is-active').checked=r.is_active!==false;
  document.getElementById('mark-as-read').checked=!!r.mark_as_read;
  document.getElementById('keep-in-inbox').checked=!!r.keep_in_inbox;
...
function clearForm(){
  ...
  document.getElementById('keep-in-inbox').checked=false;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if(editId){
    try{
      ...
      const rule=all.find(r=>String(r.id)===String(editId));
      if(rule)setForm(rule);
    }catch(e){console.error(e);}
  } else if(labelParam){
    document.getElementById('label-name').value=decodeURIComponent(labelParam);
  }
  ...
  document.getElementById('rule-form')?.addEventListener('submit',async(e)=>{
    ...
    if(!data.from_contains&&!data.subject_contains&&!data.body_contains){
      showStatus('rule-status','At least one match condition (From, Subject, or Body) is required.','error');return;
    }
    ...
    btn.disabled=true;
    btn.innerHTML='<span class="spinner-border spinner-sm me-2"></span>Saving…';
    try{
      ...
      showStatus('rule-status',
        (editingId?'✓ Rule updated!':'✓ Rule saved!')+' <a href="/rule-list" style="color:#58d68d">View all rules</a>',
        'success'
      );
